...
const requiredString = string.then(check(s => s.length > 0, "required"));
...
const optioinalPhone = byDefault("-").then(string).then(checkPhone);
...
const email = string.then(check(v.isEmail, "not_a_email"));
...
const requiredString = string.then(check(s => s.length > 0, "required"));
const optionalString = byDefault("").then(string);
const requiredEmail = string.then(check(v.isEmail, "not_a_email"))
const strongPassword = string.then(check(isStrongPassword, "to_simple"));
const optionalPhone = optionalString.then(check(v.isMobilePhone, "invalid_format"))
...
const phone = string.then(check(v.isMobilePhone, "invalid_format"))

const phoneNumberList = array(phone.then(check(x => x.length > 0, "no_phones_provided"));
```
...
If you ommit part `.then(check(x => x.length > 0, "no_phones_provided"));` then error type will be an array of `string | undefiend`:
...
const phone = string.then(check(v.isMobilePhone, "invalid_phone"));
const email = string.then(check(v.isEmail, "invalid_email"));
...
const complexModel = shape({
    ...
    subModels: array(shape({
        ...
        active: byDefault(true).then(boolean),
    }))
})
```
...
```javascript
chain1 = A.then(B).then(C)
```

### Nested style

```javascript
chain2 = A.then(B.then(C))
```
